#!/usr/bin/env python3
"""Download full PDB files from RCSB for the top100 dataset.

Reads the top100 chain list CSV and downloads each unique PDB structure
into a two-level directory structure (prefix/pdbid/pdbid.pdb).

Usage:
    python top100/download_pdbs.py top100/top100_chain_list.csv DST_DIR [-j JOBS]
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Download full PDB files from RCSB for the top100 dataset"
    )
    parser.add_argument(
    ...
    print(f"Done: {done - errors} downloaded, {errors} errors")
